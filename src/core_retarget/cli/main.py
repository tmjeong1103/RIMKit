...
from core_retarget import __version__
...
def _run_serve(args: argparse.Namespace) -> int:
    try:
        from core_retarget.web.server import serve
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="core-retarget",
        description="Contact-aware SOMA-to-humanoid motion retargeting.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"core-retarget: error: {exc}", file=sys.stderr)
        return 2
    return 2
